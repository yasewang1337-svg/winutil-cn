...
- 用户已登录 Codex 内置浏览器；GitHub CLI 尚未登录。CLI `auth login --scopes workflow` 被自动审批拒绝，理由是缺少对独立登录和账号范围权限的明确授权；已提出授权问题，等待答复，不绕过审批。
...
1. 确认 GitHub CLI 授权答复；若允许，启动官方登录，由用户确认 GitHub 权限页。不得提取浏览器凭据。
2. 推送维护分支、创建并核对 PR 的远端 CI，通过后合并；核验 3 个依赖 PR 状态与新发布。
3. Issue #6 说明故障链与版本，待原报告环境确认；Issue #5 保留开放，说明上游迁移范围（`docs/MAINTENANCE.md`）。
