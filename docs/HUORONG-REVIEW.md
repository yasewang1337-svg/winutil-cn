...
被删除产物未能取得 SHA256，未恢复隔离文件。根目录 EXE 仍是先前发布版，不能作为本次被拦截样本，也不能用其哈希替代本次样本哈希。两份根目录旧PS1在06:44读取检查时也被拒绝访问，随后复查已不存在；本轮没有替换或交付新版产物。
...
目前只能确认历史告警和当前构建阻断，尚不能确认具体命中特征或断言误报。最新双 PowerShell 引擎各 311 项隔离测试通过，前续启动器无害夹具回归通过；这些软件测试不能替代杀毒检测或厂商复核。
